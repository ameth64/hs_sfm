...
class TestBAGCPConstrainedJacobianMatrixCalculator
{
public:
  ...
  typedef typename BAFFDJacobianMatrix::NaiveJacobianMatrix NaiveJacobianMatrix;
                  ...
                  const XVector& x, bool with_ffd)
  {
    ...
    std::cout<<"BA FFD took "
             <<Scalar(ba_ffd_end - ba_ffd_begin) /
               Scalar(CLOCKS_PER_SEC / 1000)<<" ms.\n";
    NaiveJacobianMatrix& analytical_naive_jacobian_matrix =
      ba_analytical_jacobian_matrix.naive_jacobian_matrix();
    NaiveJacobianMatrix& ffd_naive_jacobian_matrix =
      ba_ffd_jacobian_matrix.naive_jacobian_matrix();

    if (analytical_naive_jacobian_matrix.number_of_cameras() !=
          ffd_naive_jacobian_matrix.number_of_cameras() ||
        analytical_naive_jacobian_matrix.number_of_points() !=
          ffd_naive_jacobian_matrix.number_of_points() ||
        analytical_naive_jacobian_matrix.camera_derivatives().size() !=
          ffd_naive_jacobian_matrix.camera_derivatives().size() ||
        analytical_naive_jacobian_matrix.point_derivatives().size() !=
          ffd_naive_jacobian_matrix.point_derivatives().size())
    {
    ...
    for (Index i = 0;
         i < Index(ffd_naive_jacobian_matrix.camera_derivatives().size());
         i++)
    {
      const typename NaiveJacobianMatrix::CameraDerivativeBlock&
        ba_analytical_camera_block =
          analytical_naive_jacobian_matrix.camera_derivatives()[i];
      const typename NaiveJacobianMatrix::CameraDerivativeBlock&
        ba_ffd_camera_block =
          ffd_naive_jacobian_matrix.camera_derivatives()[i];
    ...
    for (Index i = 0;
         i < Index(ffd_naive_jacobian_matrix.point_derivatives().size());
         i++)
    {
      const typename NaiveJacobianMatrix::PointDerivativeBlock&
        ba_analytical_point_block =
          analytical_naive_jacobian_matrix.point_derivatives()[i];
      const typename NaiveJacobianMatrix::PointDerivativeBlock&
        ba_ffd_point_block =
          ffd_naive_jacobian_matrix.point_derivatives()[i];
...
TEST(TestBAGCPConstrainedJacobianMatrixCalculator, SmallDataTest)
{
  ...
  typedef hs::sfm::ba::BAGCPConstrainedSyntheticDataGenerator<Scalar,
                                                              ImageDimension>
          DataGenerator;
  typedef TestBAGCPConstrainedJacobianMatrixCalculator<Scalar> Test;
...
TEST(TestBAGCPConstrainedJacobianMatrixCalculator, BigDataTest)
{
  typedef double Scalar;
  typedef size_t ImageDimension;
