...
public:
                  ...
                  ExtrinsicParamsContainer& extrinsic_params_set) const
  {
    ...
    for(int image_count=0;image_count!=pattern_views.size();image_count++)
    {
      realPoint.clear();
      cornerPoint.clear();
      for(int point_count=0;
          point_count!=pattern_views[image_count].size();
          point_count++)
      {
        const Point& point = pattern_views[image_count][point_count].second;
        realPoint.push_back(cv::Point3f(float(point[0]),
                                        float(point[1]),
                                        0.0f));
        const Key& key = pattern_views[image_count][point_count].first;
        cornerPoint.push_back(cv::Point2f(float(key[0]),
                                          float(key[1])));
      }
    ...
    cv::calibrateCamera(objectPoint,imagePoint,
                        cv::Size(int(image_width), int(image_higth)),
                        cameraMatrix,distCoeffs,rvecs,tvecs);

    intrinsic_params = IntrinsicParams(Scalar(cameraMatrix.at<double>(0, 0)), 
      Scalar(0), Scalar(cameraMatrix.at<double>(0, 2)),
      (Scalar)cameraMatrix.at<double>(1, 2), (Scalar)1,
      (Scalar)distCoeffs.at<double>(0, 0),
      (Scalar)distCoeffs.at<double>(0, 1),
      (Scalar)distCoeffs.at<double>(0, 4),
      (Scalar)distCoeffs.at<double>(0, 2),
      (Scalar)distCoeffs.at<double>(0, 3));
